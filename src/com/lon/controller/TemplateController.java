...
public class TemplateController {
	...
	@RequestMapping("/add")
	public void add(Template user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			user.setId(UUID.randomUUID().toString().replaceAll("-", ""));
			out = response.getWriter();
			templateService.add(user);
			jo.put("state", 0);
			jo.put("msg", "成功新增记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void update(Template user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			out = response.getWriter();
			templateService.update(user);
			jo.put("state", 0);
			jo.put("msg", "成功修改记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void queryByPage(String page) {
		...
		List<TemplateVo> list = templateService.tqueryByPage(sp, pageSize);
		try {
