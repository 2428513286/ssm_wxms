...
import com.lon.entity.PublicNumberFans;
import com.lon.entity.User;
import com.lon.service.PublicNumberFansService;
...
public class PublicNumberFansController {
	...
	@RequestMapping("/add")
	public void add(PublicNumberFans user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			user.setId(UUID.randomUUID().toString().replaceAll("-", ""));
			out = response.getWriter();
			publicNumberFansService.add(user);
			jo.put("state", 0);
			jo.put("msg", "成功新增记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void update(PublicNumberFans user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			out = response.getWriter();
			publicNumberFansService.update(user);
			jo.put("state", 0);
			jo.put("msg", "成功修改记录");
		}catch(Exception e) {
			jo.put("state", -1);
