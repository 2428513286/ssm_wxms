...
public class PublicNumberCheckController {
	...
	@RequestMapping("/add")
	public void add(PublicNumberCheck user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			user.setId(UUID.randomUUID().toString().replaceAll("-", ""));
			out = response.getWriter();
			publicNumberCheckService.add(user);
			jo.put("state", 0);
			jo.put("msg", "成功新增记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void deleteMore(String ids) {
		...
		try {
			out=response.getWriter();
			publicNumberCheckService.deleteMore(User.class, ids);
			jo.put("state", 0);
			jo.put("msg", "成功删除记录");
		} catch (Exception e) {
			jo.put("state", -1);
	...
	public void update(PublicNumberCheck user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			out = response.getWriter();
			publicNumberCheckService.update(user);
			jo.put("state", 0);
			jo.put("msg", "成功修改记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void queryByPage(String page) {
		...
		List<PublicNumberCheckVo> list = publicNumberCheckService.uqueryByPage(sp, pageSize);
		try {
