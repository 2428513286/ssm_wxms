...
import com.lon.service.InfoContentCheckService;
...
public class InfoContentCheckController {
	...
	@RequestMapping("/add")
	public void add(InfoContentCheck user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			out = response.getWriter();
			infoContentCheckService.add(user);
			jo.put("state", 0);
			jo.put("msg", "成功新增记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void update(InfoContentCheck user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			out = response.getWriter();
			infoContentCheckService.update(user);
			jo.put("state", 0);
			jo.put("msg", "成功修改记录");
		}catch(Exception e) {
			jo.put("state", -1);
