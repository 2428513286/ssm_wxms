...
import com.lon.entity.InfoContent;
import com.lon.entity.User;
import com.lon.service.InfoContentService;
...
public class InfoContentController {
	...
	@RequestMapping("/add")
	public void add(InfoContent user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			user.setId(UUID.randomUUID().toString().replaceAll("-", ""));
			out = response.getWriter();
			infoContentService.add(user);
			jo.put("state", 0);
			jo.put("msg", "成功新增记录");
		}catch(Exception e) {
			jo.put("state", -1);
	...
	public void update(InfoContent user) {
		JSONObject jo = new JSONObject();
		PrintWriter out = null;
		try {
			out = response.getWriter();
			infoContentService.update(user);
			jo.put("state", 0);
			jo.put("msg", "成功修改记录");
		}catch(Exception e) {
			jo.put("state", -1);
